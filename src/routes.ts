...
export async function loginRoute(req: Request, res: Response) {
  ...
  res.cookie('token', token, { httpOnly: true, secure: true,sameSite: "none", maxAge: 2 * 24 * 60 * 60 * 1000 }); // Set the token cookie
...
export async function logoutRoute(req: Request, res: Response) {
  const secure = process.env.NODE_ENV === 'production';
  res.clearCookie('token', {httpOnly: true, secure: true, sameSite: "none", maxAge: 2 * 24 * 60 * 60 * 1000 }); // Clear the token cookie
